    ...
    @include('components.task-header')
        <main class="text-custom-gray flex flex-grow">
            <div class="container px-4 sm:px-6 md:px-8 lg:px-24 py-3 sm:py-4 md:py-5 mx-auto">
                ...
                @endif
                <!-- タブ部分 -->
                <div id="task-container" data-selected-child-id="{{ $selectedChildId }}">
                    @if($children->count() > 0)
                        {{-- タブ切り替えボタン --}}
                        <div class="flex flex-row justify-between sm:justify-center gap-1 sm:gap-4 text-white font-medium z-10 relative">
                            @foreach ($children as $index => $child)
                                <button data-tab="{{ $index }}" data-child-id="{{ $child->id }}" class="js-tab-button flex-1 sm:flex-none sm:w-[180px] md:w-[216px] px-2 sm:px-6 py-2 rounded-t-lg {{ $child->id === $selectedChildId ? 'bg-custom-pink' : 'bg-custom-blue '}} text-xs sm:text-base">{{ $child->child_name }}</button>
                            @endforeach
                        </div>
                        <!-- タスク表示部分 -->
                        @foreach ($children as $index => $child)
                            <div data-panel="{{ $index }}" class="js-tab-panel1 flex flex-col justify-between bg-gray-100 border-custom-gray text-center font-medium h-[60vh] border-2 overflow-y-auto {{ $child->id === $selectedChildId ? '' : 'hidden' }}">
